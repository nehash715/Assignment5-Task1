...
showCalendar=()=>{
    ...
    if(+a?.date.substring(4,6)<10){
        console.log('true')
         this.setState({month:a?.date.substring(6,9)})
    }
    else{

    
    console.log("a",   a?.date.substring(7))
    this.setState({month:a?.date.substring(7,10)})
    }
}
...
let {montharr,year,month,domestic,deptfrom,chosereturn,destination}=this.state
  let index=montharr.findIndex((p)=>{
        return p==month.substring(0,3)
       })
       //console.log(month.substring(0,3),index)
 let a=domestic.find((p)=>{
...
let returndate
if(today<10){
     returndate=`0${today1.getDate()},${montharr[today1.getMonth()]},${year}`
}
else{
    returndate=`${today1.getDate()},${montharr[today1.getMonth()]},${year}`
}
...
let d=returndate.substring(0,2)
console.log(returndate.substring(3,6))
/*if(d<10){
    console.log("d<10",   returndate.substring(2,5))
    this.setState({monthReturn:returndate.substring(2,5)})
}
else{
    console.log("d>10",  returndate.substring(3,6))
    this.setState({monthReturn:returndate.substring(4,7)})
}*/
this.setState({monthReturn:returndate.substring(3,6)})
...
handleReturnDate=(d,month,y)=>{
    ...
    if(d<10){
        s1.returndate=`0${d},${s1.montharr[month]},${y}`
    }
    else{
...
handlecheckIndate=(d,month,y)=>{
    ...
    s1.checkInYear=y
    if(d<10){
        s1.checkIndate=`0${d},${s1.montharr[month]},${y}`
    }
    else{
        s1.checkIndate=`${d},${s1.montharr[month]},${y}`
    }
...
handlecheckOutdate=(d,month,y)=>{
    ...
    s1.checkOutYear=y
    if(d<10){
        s1.checkOutdate=`0${d},${s1.montharr[month]},${y}`
    }
    else{
        s1.checkOutdate=`${d},${s1.montharr[month]},${y}`
    }
...
searchRoom=()=>{
  ...
  render() {
   ...
   checkIndate,checkOutdate,checkInCalShow,checkOutCalShow,hoteluparrow,roomNo,room,roomarr,yearReturn,
   checkInYear,checkOutYear  }=this.state
...
<span className="fa-layers fa-fw fa-lg" style={{marginRight:50}}>
  ...
  <FontAwesomeIcon className='icons'  icon={faRightLong}    transform="shrink-6" inverse />
       ...
       <div className='row'>
        <div className="col-6">
          ...
          {/*console.log(a?.date.substring(6))*/}
           ...
           {showCalendar==true?<Calendargt  index={index} year={year} selectDate={this.handleDate} />:""}
       ...
       {showReturnCalendar==true?<CalendarReturn index={indexreturn} year={yearReturn}  selectDate={this.handleReturnDate}/>:""}
        </div>
       </div>
       <div className='row'>
        <div className="col-5">
...
<h6>Select City,Location,Hotel Name</h6>
    ...
    <div className='col-6'>
        ...
        {checkInCalShow==true?<CheckInCal index={checkInIndex} year={checkInYear} selectDate={this.handlecheckIndate}/>:""}
    ...
    <div className='col-6'>
       ...
       {checkOutCalShow==true?<CheckOutCal index={chechoutIndex} year={checkOutYear} selectDate={this.handlecheckOutdate}/>:""}
    </div>
